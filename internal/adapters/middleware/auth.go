...
func (m *AuthMiddleware) Authenticate() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if authHeader == "" {
			logger.Debug("Missing authorization header", zap.String("request_id", GetRequestID(c)))
		...
		if err != nil {
			logger.Debug("Invalid authorization header format", zap.String("request_id", GetRequestID(c)))
			return http.NoAuth(c)
		}
